...
$(document).ready(function () {
  ...
  const updateCurrentWeather = (data, cityName) => {
    ...
    if (data.main) {
      temperatureElement.text(`Temperature: ${data.main.temp} °C`);
  ...
  const updateFiveDayForecast = (forecastData) => {
    ...
    forecastData.list.forEach((dayData) => {
      const forecastDay = dayjs.unix(dayData.dt).format('YYYY-MM-DD');
      if (!uniqueDays.includes(forecastDay) && uniqueDays.length < 5) {
